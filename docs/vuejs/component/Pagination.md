...
---
## Giới thiệu
Component hỗ trợ phân trang khi sử dụng Laravel + Vue
...
# Ví dụ cách dùng: ở đây với Inertia nữa

``` vue
<!-- Phân trang -->
    <Pagination :meta="customers" @page-change="handlePageChange"
      @items-per-page-change="handleItemsPerPageChange" />

      
const handlePageChange = (page) => {
  router.get(route('sales.customers.index'), { ...props.filters, page }, { preserveState: true })
}

const handleItemsPerPageChange = (limit) => {
  router.get(route('sales.customers.index'), { ...props.filters, per_page: limit, page: 1 }, { preserveState: true })
}
